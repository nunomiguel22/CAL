...
/**
 * Classe que guarda o grafo e o representa. Todas as suas funções retornam um
 * booleano a indicar se a sua execução decorreu ou não com sucesso.
 */
class GraphViewer {
 public:
  /**
  ...
  ~GraphViewer();
  /**
   * Variável que guarda a próxima porta que o programa vai usar. O valor
   * inicial é 7772.
   */
  static short port;
   ...
   * @param dynamic Booleano que determina se a localização dos nós é
   * automaticamente. determinado pelo programa (true) ou se deve ser
   * determinado pelo utilizador (false).
   */
  GraphViewer(int width, int height, bool port_n);

  /**
   * Construtor que cria um novo grafo, utilizando uma porta especificada pelo
   ...
   * automaticamente. determinado pelo programa (true) ou se deve ser
   * determinado pelo utilizador (false).
   * @param port_n Inteiro que determina a porta a utilizar. Deve-se ter cuidado
   * para não utilizar uma porta já usada por outro programa ou pelo sistema.
   */
  ...
  /**
   * Acrescenta um nó à representação do grafo, numa posição ao critério do
   * programa.
   * @param id Identificador único do nó.
   */
  bool addNode(int id);
   ...
   * @param color Nova cor da aresta, utilizar as constantes definidas no
   * graphviewer.h para conveniência.
   */
   ...
   * @param color Nova cor do nó, utilizar as constantes definidas no
   * graphviewer.h para conveniência.
   */
   ...
   * @param color Nova cor das arestas, utilizar as constantes definidas no
   * graphviewer.h para conveniência.
   */
  bool defineEdgeColor(string color);
  /**
   * Função que define a cor global dos nós.
   * @param color Nova cor dos nós, utilizar as constantes definidas no
   * graphviewer.h para conveniência.
   */
  ...
  /**
   * Função que define o peso de uma aresta na representação do grafo, a ser
   * visualizado como w: valor_do_peso, seguido de qualquer outro texto
   * associado à aresta.
  ...
  /**
   * Função que define o fluxo de uma aresta na representação do grafo, a ser
   * visualizado como f: valor_do_fluxo, precedido pelo peso e seguido por texto
